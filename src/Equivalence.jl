function getEquivalentLoad(Load::Vector{Float64}, NC::Matrix{Float64}, EC::Vector{<:Element}, NE::Integer, U::Vector{Float64})
    ...
    for i in 1:NE
        ...
        if EC[i] isa RigidLink
            ...
            # Compute the equvalent loads:
            S = -1
            F_x_eq = S * (F_x_1 + F_x_2)
            F_y_eq = S * (F_y_1 + F_y_2)
            F_z_eq = S * (F_z_1 + F_z_2)
            M_x_eq = S * (M_x_1 + M_x_2 - L_z * F_y_2 + L_y * F_z_2)
            M_y_eq = S * (M_y_1 + M_y_2 - L_x * F_z_2 + L_z * F_x_2)
            M_z_eq = S * (M_z_1 + M_z_2 - L_y * F_x_2 + L_x * F_y_2)
